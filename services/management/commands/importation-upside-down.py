...
def clean_entities_without_name(self):
    ...
    self.stdout.write(
        self.style.MIGRATE_HEADING("%s INFO: Cleanning OSM Entities" % datetime.now()))
    ...
    rows = 1
    while not rows == 0:
        fathers_ways = Way.objects.only('id').filter(relation_reference_id__isnull=True, checked_name=False)[:1000]
        for way in fathers_ways:
            tag_list = Tag.objects.filter(reference=way.id)
            name, _ = get_name_shape(tag_list)
            if not name:
                way.delete()
                count_nodes += Node.objects.only('id').filter(way_reference=way.id).count()
                Node.objects.filter(way_reference=way.id).delete()

                count += 1
        fathers_ways.update(checked_name=True)
        rows = len(fathers_ways)
    ...
    while not rows == 0:
        ...
        for relation in fathers_relations:
            ...
            if not name:
                count_nodes += Node.objects.only('id').filter(relation_reference=relation.id).count()
                Node.objects.filter(relation_reference=relation.id).delete()

                ways = Way.objects.only('id').filter(relation_reference=relation.id)
