...
class GeoLocation:
    ...
    def _check_bounds(self):
        if self.rad_lat < GeoLocation.MIN_LAT or self.rad_lat > GeoLocation.MAX_LAT \
                or self.rad_lon < GeoLocation.MIN_LON or self.rad_lon > GeoLocation.MAX_LON:
    ...
    def get_degres_coordinates(self):
        """

        :return: the coordinates of the geolation in degres
        """
        return self.deg_lat, self.deg_lon

    def get_radians_coordinates(self):
        """

        :return: the coordinates of the class in radians
        """
        return self.rad_lat, self.rad_lon
    ...
    def meridian180_within_distance(self, distance, radius=EARTH_RADIUS):
        (min_loc, max_loc) = self.bounding_locations(distance, radius)
    ...
    @staticmethod
    def get_angular_radius(distance, radius=EARTH_RADIUS ):
